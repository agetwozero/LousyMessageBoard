...
debconf-set-selections <<< 'mysql-server mysql-server/root_password password pindakaas'
debconf-set-selections <<< 'mysql-server mysql-server/root_password_again password pindakaas'
apt-get update
...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  config.vm.provider "virtualbox" do |vb|
    ...
    vb.customize ["modifyvm", :id, "--name", "LousyMessageboard"]
  end
end
