<?php
/**
 * https://github.com/olegkrivtsov/using-zf3-book-samples/blob/master/userdemo/module/User/src/Service/UserManager.php
 */

namespace FrozenAuth\Service;

use FrozenAuth\Model\UserStatusTypes;
use Zend\Crypt\Password\Bcrypt;
use Zend\Math\Rand;
use FrozenAuth\Model\User;
...
class UserManager
{
    ...
    public function createAdminUserIfNotExists()
    {
        ...
        if ($user==null) {
            ...
            $user->setPassword($passwordHash);
            $user->setStatus(UserStatusTypes::STATUS_ACTIVE);
    ...
    {
        // Generate a token.
        //removed a true from randgetstring
        $token = Rand::getString(32, '0123456789abcdefghijklmnopqrstuvwxyz');
        $user->setPasswordResetToken($token);

        $currentDate = date('Y-m-d H:i:s');
        $user->setPwdResetTokenCreationDate($currentDate);
    ...
    {
       User:$user = $this->entityManager->getRepository(User::class)
        ...
        $tokenCreationDate = $user->getPwdResetTokenCreationDate();
